...
合并说明：
- 基础代码基于 NO.6（修复了 _enforce_memory_cap 的 SQL 兜底逻辑）
- 对话历史读写采用 NO.5 的短回复过滤防护（防止垃圾数据污染）
- 交换记录存储采用 NO.5 的过滤逻辑
"""
import sqlite3
import json
...
class MeaMemory:
    ...
    def add_chat(self, role: str, content: str, mood: str = "neutral"):
        # 合并版：采用 NO.5 的短回复过滤，避免垃圾数据写入
        if role == "mea" and len(content) < 100 and "DISPLAY" not in content and "__MEAPET_SILENT__" not in content:
            log.debug(f"[DB] 跳过写入短回复 (len={len(content)})")
            return
    ...
    def get_recent_chats(self, limit: int = 20, exclude_summarized: bool = True, filter_short: bool = True) -> List[Dict]:
        """
        获取最近对话记录。
        :param limit: 最大返回条数
        :param exclude_summarized: 是否排除已摘要的
        :param filter_short: 是否过滤掉 assistant 角色下短于 100 字符且不含协议标记的记录（合并版保留 NO.5 行为）
        """
        with self._lock:
            c = self.conn.cursor()
            fetch_limit = limit * 2 if filter_short else limit
        ...
        for r in reversed(rows):
            role = r["role"]
            content = r["content"]
            if filter_short and role == "mea" and len(content) < 100 and "DISPLAY" not in content and "__MEAPET_SILENT__" not in content:
                continue
            result.append({
                "role": role,
                "content": content,
    ...
    # ========================
    # 记忆总量上限（已修复 SQL 语法错误，采用 NO.6 逻辑）
    # ========================
    def _enforce_memory_cap(self, avoid_type: Tuple[str, ...] = ()) -> int:
        ...
        with self._lock:
            ...
            if not ids:
                # 重要度裁剪不够，直接按 last_recalled 裁。使用正确 WHERE 子句（NO.6 修复）
                where_expr = ""
                if avoid_type:
                    placeholders = ", ".join("?" for _ in avoid_type)
    ...
    def store_chat_exchange(self, user_msg: str, mea_reply: str):
        # 合并版：采用 NO.5 的短回复过滤，防止无意义的交换记录进入长期记忆
        if len(mea_reply) < 100 and "DISPLAY" not in mea_reply and "__MEAPET_SILENT__" not in mea_reply:
            log.debug(f"[DB] 跳过存储短回复 exchange (len={len(mea_reply)})")
            return
    ...
    def build_context_prompt(self, current_query: str = "") -> str:
        ...
        if not search_query:
            recent = self.get_recent_chats(2, exclude_summarized=True, filter_short=True)
            if recent:
        ...
        # 最近未摘要对话（使用带过滤的读取，避免短回复污染上下文）
        recent = self.get_recent_chats(8, exclude_summarized=True, filter_short=True)
        if recent:
            lines.append("")
