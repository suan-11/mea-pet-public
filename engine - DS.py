...
class ChatEngine:
    ...
    def _commit_direct_turn(self, result, raw_text: str = "") -> None:
        """存储完整协议格式到历史，如果找不到完整协议则不存储（避免污染）。"""
        log.info(f"[_commit] raw_text 长度={len(raw_text)}, 前100字符: {raw_text[:100] if raw_text else '(空)'}")
        ...
        if raw_text and raw_text.strip():
            ...
            # 没有原始输出，放弃存储
            log.warning(f"[_commit] raw_text 为空，放弃存储")
            return
