...
_DIRECT_OUTPUT_INSTRUCTION = (
    ...
    "</MEAPET_SEGMENT>\n"
    "最后输出<MEAPET_DONE />。\n"
    "如果输出不符合此格式，系统将无法解析。请严格按照上述格式输出。"
)
...
class DirectConversationAdapter:
    ...
    async def stream_turn(self, request: AgentTurnRequest) -> AsyncIterator[object]:
        ...
        try:
            ...
            if raw_text.strip():
                log.info(f"[direct] 模型返回原文预览 (前500字符): {raw_text[:500]}")
                log.debug(f"[direct] 模型返回完整原文: {raw_text}")
            if raw_text.strip():
                log.info(
            ...
            if not is_vision:
                self.engine._commit_direct_turn(result, raw_text)
